    	...
    	(letrec
			((fold-left-rec
					(lambda (proc acc lst)
						(if (null? lst)
						acc
						(fold-left-rec proc (proc acc (car lst)) (cdr lst)))
					)
			))
			(lambda (proc acc lst)
				(fold-left-rec proc acc lst)
			)
    	)
	)
)
...
(define reverse
	(let 
    	...
    	(letrec
			((reverse-rec
				(lambda (lst reversed-lst)
					(if (null? lst)
						reversed-lst
						(reverse-rec (cdr lst) (cons (car lst) reversed-lst))
					)
				)
			))
        	(lambda (lst) (reverse-rec lst))
    	)
	)
)
		...
		(letrec
			((fold-right-rec
				(lambda (proc acc lst)
					(if (null? lst)
					acc
					(fold-right-rec proc (proc (car lst) acc) (cdr lst)))
				)
			))
			(lambda (proc acc lst)
				(fold-right-rec proc acc (reverse lst))
			)
		)
  	)
)
...
(define remove-last
  (let ((null? null?) (car car) (cdr cdr) (cons cons))
    (letrec ((remove-last-rec
              (lambda (lst)
                (if (null? (cdr lst))
                    '()
                    (cons (car lst) (remove-last-rec (cdr lst)))))))
      remove-last-rec)))
...
(define cons*
	(lambda (expr . exprs)
		(let ((null? null?) (cons cons))
			(if (null? exprs)
				expr
				(fold-right cons (get-acc exprs) (remove-last (cons expr exprs))))
		)
	)
)
